...
RSpec.describe Api::V1::ListingsController, type: :controller do
  ...
  describe 'POST#create' do
    ...
    it 'returns json of destination for the new listing' do
      ...
      result = parsed_response['destination']
  ...
  describe 'PATCH#update' do
    ...
    it 'returns the json for the updated listing' do
      ...
      expect(parsed_response['id']).to eq listing.id
      expect(parsed_response['destination_id']).to eq listing.destination_id
      expect(parsed_response['user_id']).to eq listing.user_id
      expect(parsed_response['visited']).to eq true
    end
  end
  ...
  describe 'DELETE#destroy' do
    ...
    it 'returns the updated list of listings for the current user' do
      ...
      expect(parsed_response[0]['id']).to eq new_listing.id
      expect(parsed_response[0]['destination_id']).to eq new_listing.destination_id
      expect(parsed_response[0]['user_id']).to eq new_listing.user_id
    end
  end
